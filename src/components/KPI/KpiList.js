...
export default class BudgetSourceList extends React.Component {
  ...
  onEdit = id => {
    ...
    axios
      ...
      .then(res => {
        const { _id, budget_source_id, kpi_type_name } = res.data;

        this.setState({
          BudgetSourceId: budget_source_id,
          BudgetSourceName: kpi_type_name,
          modal: true,
          ID: _id,
          isForm: 'edit',
          validationError: '',
          validationError1: '',
          header: 'แก้ไขข้อมูลประเภทตัวชี้วัด',
        });
      });
  };
  handleInputChange = e => {
    if (e.target.name == 'BudgetSourceName') {
      this.setState({
        validationError: '',
      });
    }
  ...
  onAdd() {
    this.setState({
      ...
      ID: '',
      validationError: '',
      header: 'เพิ่มข้อมูลประเภทตัวชี้วัด',
    });
  }
  handleSubmit = e => {
    e.preventDefault();
  ...
  handleSubmit1 = e => {
    e.preventDefault();
    if (this.state.BudgetSourceName.trim()) {
      axios
        .post(
          process.env.REACT_APP_SOURCE_URL + '/dataKpiTypes',
          {
            kpi_type_name: this.state.BudgetSourceName.trim(),
          },
        ...
        .then(res => {
          ...
          setTimeout(() => (window.location.href = '/kpilist/'), 1000);
        })
        .catch(err => {
          if (err.response.data == 'Please Check your kpi type name!') {
            const validationError1 = 'ชื่อประเภทตัวชี้วัดซ้ำ กรุณากรอกใหม่';
    ...
    if (this.state.BudgetSourceName.trim()) {
      axios
        ...
        .then(res => {
          ...
          setTimeout(
            () => (window.location.href = '/kpilist/' + this.state.selected),
            1000,
          );
        })
        .catch(err => {
              ...
              >
                ...
                <ModalBody>
                  <Form onSubmit={this.handleSubmit}>
                    {/* <FormGroup>
                      <Label sm={12}>
                        รหัสข้อมูลแหล่งงบประมาณ <font color="red">*</font>
                      </Label>
                      <Col sm={12}>
                        <Input
                          type="text"
                          name="BudgetSourceId"
                          placeholder="รหัสข้อมูลแหล่งงบประมาณ"
                          onChange={this.handleInputChange}
                    ...
                    <FormGroup>
                      <Label sm={12}>
                        ชื่อประเภทตัวชี้วัด <font color="red">*</font>
                      </Label>
                      <Col sm={12}>
                        <Input
                          type="textarea"
                          name="BudgetSourceName"
                          placeholder="ชื่อประเภทตัวชี้วัด"
                          onChange={this.handleInputChange}
