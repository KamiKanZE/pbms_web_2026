...
export default class ReportRiskData extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
  ...
  hideRisk = id => {
    //document.getElementById(id).classList.add('hightlight');
    //document.getElementById(id).classList.remove('hightlight');
    for (let index = 1; index <= 5; index++) {
      for (let index1 = 1; index1 <= 5; index1++) {
        if (index.toString() + index1.toString() !== id) {
      ...
      <>
        <Row>
          ...
          <Col>
            ...
            <div style={{ display: 'flex', justifyContent: 'right' }}>
              <NavLink
                to={`/ReportActivities/ReportDetail3/${this.state.projectId}`}
              >
                {'ข้อมูลตัวชี้วัด <<'}
              </NavLink>
