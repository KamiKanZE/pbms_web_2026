...
export default class EditProjectKPIData extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
      ...
      <>
        <Row>
          ...
          <Col>
            ...
            <div style={{ display: 'flex', justifyContent: 'right' }}>
              <NavLink to={`/AllProject/EditProject2/${this.state.projectId}`}>
                {'ข้อมูลกิจกรรม <<'}
              </NavLink>
              <div className="box-10">{'หน้าก่อน'}</div>
              <div className="box-10">{'หน้าถัดไป'}</div>
              <NavLink to={`/AllProject/EditProject4/${this.state.projectId}`}>
                {'>> ข้อมูลความเสี่ยง'}
              </NavLink>
            </div>
