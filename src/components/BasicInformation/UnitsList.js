...
export default class UnitsList extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dataType: '2',
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataUnits/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataUnits/' + id)
      .then(res => {
        ...
        // .map(id => ({'PlanID':
        //   id.main_unit_id, 'PlanName':id.main_unit_name
        // }));
        this.setState({
          // PlanID: unit_id,
          // PlanName: unit_name,
    ...
    if (
      ...
      axios
        .post(
          ...
          {
            // unit_id: this.state.PlanID.trim(),
            // unit_name: this.state.PlanName.trim(),
            unit_name: this.state.dataName.trim(),
            // unit_detail: this.state.dataDetail,
            // unit_type: this.state.dataType,
        ...
        .catch(err => {
          if (
            err.response.data === 'This unit_name already exists!' ||
      ...
      axios
        .patch(
          process.env.REACT_APP_SOURCE_URL + '/dataUnits/' + ID,
          {
            ...
            // unit_detail: this.state.dataDetail,
            // unit_type: this.state.dataType,
            unit_status: this.state.status.toString(),
          },
          { headers: { Authorization: `Bearer ${token_id}` } },
        )
        .then(res => {
        ...
        .catch(err => {
          if (err.response.data === 'This unit_name already exists!') {
            const validationError1 = 'ชื่อข้อมูลซ้ำ กรุณากรอกใหม่';
            ...
            <Table responsive borderless id="table1">
              ...
              <tbody>
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data
                    //.filter(item => item.unit_type === this.state.dataType)
                    .map((id, i) => (
                      <tr key={'table' + i}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <Tooltip title="แก้ไข">
                            <span
                              ...
                              onClick={() => this.onEdit(id.unit_id)}
                            >
                          ...
                          <Tooltip title="ลบ">
                            <span
                              ...
                              onClick={() => this.testAxiosDELETE(id.unit_id)}
                            >
