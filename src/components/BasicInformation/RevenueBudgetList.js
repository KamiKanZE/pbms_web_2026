...
export default class RevenueBudgetList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataRevenueTypes/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataRevenueTypes/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      ...
      .then(res => {
        ...
        // .map(id => ({'PlanID':
        //   id.main_revenue_type_id, 'PlanName':id.main_revenue_type_name
        // }));
        this.setState({
          // PlanID: revenue_type_id,
          // PlanName: revenue_type_name,
          modal: true,
          ID: revenue_type_id,
          isForm: 'edit',
          dataName: revenue_type_name,
          // dataDetail: revenue_type_detail,
          // startYear: development_goal_start,
          // endYear: development_goal_finish,
          status: revenue_type_status,
          validationError: '',
          validationError1: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      axios
        .post(
          process.env.REACT_APP_SOURCE_URL + '/dataRevenueTypes',
          {
            // revenue_type_id: this.state.PlanID.trim(),
            // revenue_type_name: this.state.PlanName.trim(),
            revenue_type_name: this.state.dataName.trim(),
            // revenue_type_detail: this.state.dataDetail,
            revenue_type_category: this.state.dataType,
        ...
        .catch(err => {
          if (err.response.data === 'This revenue_type_name already exists!') {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      axios
        .patch(
          process.env.REACT_APP_SOURCE_URL + '/dataRevenueTypes/' + ID,
          {
            // revenue_type_id: this.state.PlanID.trim(),
            // revenue_type_name: this.state.PlanName.trim(),
            revenue_type_name: this.state.dataName.trim(),
            // revenue_type_detail: this.state.dataDetail,
            revenue_type_category: this.state.dataType,
            revenue_type_status: this.state.status,
          },
          { headers: { Authorization: `Bearer ${token_id}` } },
        )
        .then(res => {
            ...
            <Table responsive borderless id="table1">
              ...
              <tbody>
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data
                    //.filter(item => item.revenue_type_category === this.state.dataType)
                    .map((id, i) => (
                      <tr key={'table' + i}>
                        ...
                        <td>{id.revenue_type_name}</td>
                        <td style={{ textAlign: 'center' }}>
                          <Tooltip title="แก้ไข">
                            <span
                              className="edit-button"
                              onClick={() => this.onEdit(id.revenue_type_id)}
                            >
                          ...
                          <Tooltip title="ลบ">
                            <span
                              ...
                              onClick={() =>
                                this.testAxiosDELETE(id.revenue_type_id)
                              }
                            >
