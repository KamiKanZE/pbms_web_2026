...
export default class MinistrySmartCityList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataSmartCities/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataSmartCities/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataSmartCities/' + id)
      .then(res => {
        ...
        this.setState({
          ...
          ID: smart_city_id,
          isForm: 'edit',
          ministryName: smart_city_name,
          ministryDetail: smart_city_detail,
          startYear: smart_city_start,
          endYear: smart_city_finish,
          status: smart_city_status,
          validationError: '',
          validationError1: '',
          validationError2: '',
          validationError3: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .post(
            process.env.REACT_APP_SOURCE_URL + '/dataSmartCities',
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              smart_city_name: this.state.ministryName.trim(),
              smart_city_detail: this.state.ministryDetail,
              smart_city_start: this.state.startYear,
              smart_city_finish: this.state.endYear,
            },
          ...
          .catch(err => {
            ...
            if (err.response.data === 'This smart_city_name already exists!') {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .patch(
            process.env.REACT_APP_SOURCE_URL + '/dataSmartCities/' + ID,
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              smart_city_name: this.state.ministryName.trim(),
              ...
              smart_city_start: this.state.startYear,
              smart_city_finish: this.state.endYear,
              smart_city_status: this.state.status,
            },
            { headers: { Authorization: `Bearer ${token_id}` } },
          )
          .then(res => {
                ...
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data.map((id, i) => (
                    <tr key={'table' + i}>
                      ...
                      <td>{id.smart_city_name}</td>
                      <td style={{ textAlign: 'center' }}>
                        {id.smart_city_start}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {id.smart_city_finish}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Tooltip title="แก้ไข">
                          <span
                            className="edit-button"
                            onClick={() => this.onEdit(id.smart_city_id)}
                          >
                        ...
                        <Tooltip title="ลบ">
                          <span
                            ...
                            onClick={() =>
                              this.testAxiosDELETE(id.smart_city_id)
                            }
                          >
