...
export default class MinistrySDGsList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataDevelopmentGoals/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataDevelopmentGoals/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataDevelopmentGoals/' + id)
      .then(res => {
        // const ButgetSource1 = res.data;
        // this.setState({ ButgetSource1 });
        const {
          development_goal_id,
          development_goal_name,
          development_goal_detail,
        ...
        this.setState({
          ...
          // PlanName: plan_name,
          modal: true,
          ID: development_goal_id,
          isForm: 'edit',
          ministryName: development_goal_name,
          ministryDetail: development_goal_detail,
          startYear: development_goal_start,
          endYear: development_goal_finish,
          status: development_goal_status,
          validationError: '',
          validationError1: '',
          validationError2: '',
          validationError3: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .post(
            process.env.REACT_APP_SOURCE_URL + '/dataDevelopmentGoals',
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              development_goal_name: this.state.ministryName.trim(),
              development_goal_detail: this.state.ministryDetail,
          ...
          .catch(err => {
            ...
            if (
              err.response.data === 'This development_goal_name already exists!'
            ) {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .patch(
            process.env.REACT_APP_SOURCE_URL + '/dataDevelopmentGoals/' + ID,
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              development_goal_name: this.state.ministryName.trim(),
              ...
              development_goal_start: this.state.startYear,
              development_goal_finish: this.state.endYear,
              development_goal_status: this.state.status,
            },
            { headers: { Authorization: `Bearer ${token_id}` } },
          )
          .then(res => {
                ...
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data.map((id, i) => (
                    <tr key={'table' + i}>
                      ...
                      <td>{id.development_goal_name}</td>
                      <td style={{ textAlign: 'center' }}>
                        {id.development_goal_start}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {id.development_goal_finish}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Tooltip title="แก้ไข">
                          <span
                            className="edit-button"
                            onClick={() => this.onEdit(id.development_goal_id)}
                          >
                        ...
                        <Tooltip title="ลบ">
                          <span
                            ...
                            onClick={() =>
                              this.testAxiosDELETE(id.development_goal_id)
                            }
                          >
