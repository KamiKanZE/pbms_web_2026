...
export default class MinistryLocalList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataLocalPlans/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataLocalPlans/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataLocalPlans/' + id)
      .then(res => {
        ...
        const {
          local_strategy_id,
          local_strategy_name,
          local_strategy_detail,
          local_strategy_start,
          local_strategy_finish,
          local_strategy_status,
        } = res.data;
        // .map(id => ({'PlanID':
        ...
        this.setState({
          ...
          isForm: 'edit',
          ministryName: local_strategy_name,
          ministryDetail: local_strategy_detail,
          startYear: local_strategy_start,
          endYear: local_strategy_finish,
          status: local_strategy_status,
          validationError: '',
          validationError1: '',
          validationError2: '',
          validationError3: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .post(
            process.env.REACT_APP_SOURCE_URL + '/dataLocalPlans',
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              local_strategy_name: this.state.ministryName.trim(),
              local_strategy_detail: this.state.ministryDetail,
              local_strategy_start: this.state.startYear,
              local_strategy_finish: this.state.endYear,
            },
          ...
          .catch(err => {
            ...
            if (
              err.response.data === 'This local_strategy_name already exists!'
            ) {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .patch(
            process.env.REACT_APP_SOURCE_URL + '/dataLocalPlans/' + ID,
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              local_strategy_name: this.state.ministryName.trim(),
              ...
              local_strategy_start: this.state.startYear,
              local_strategy_finish: this.state.endYear,
              local_strategy_status: this.state.status,
            },
            { headers: { Authorization: `Bearer ${token_id}` } },
          )
          .then(res => {
                ...
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data.map((id, i) => (
                    <tr key={'table' + i}>
                      ...
                      <td>{id.local_strategy_name}</td>
                      <td style={{ textAlign: 'center' }}>
                        {id.local_strategy_start}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {id.local_strategy_finish}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Tooltip title="แก้ไข">
                          <span
                            className="edit-button"
                            onClick={() => this.onEdit(id.local_strategy_id)}
                          >
                        ...
                        <Tooltip title="ลบ">
                          <span
                            ...
                            onClick={() =>
                              this.testAxiosDELETE(id.local_strategy_id)
                            }
                          >
