...
export default class MinistryMainPlanList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataMasterPlans/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataMasterPlans/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataMasterPlans/' + id)
      .then(res => {
        ...
        this.setState({
          ...
          ID: master_plan_id,
          isForm: 'edit',
          ministryName: master_plan_name,
          ministryDetail: master_plan_detail,
          startYear: master_plan_start,
          endYear: master_plan_finish,
          status: master_plan_status,
          validationError: '',
          validationError1: '',
          validationError2: '',
          validationError3: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .post(
            process.env.REACT_APP_SOURCE_URL + '/dataMasterPlans',
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              master_plan_name: this.state.ministryName.trim(),
              master_plan_detail: this.state.ministryDetail,
              master_plan_start: this.state.startYear,
              master_plan_finish: this.state.endYear,
            },
          ...
          .catch(err => {
            ...
            if (err.response.data === 'This master_plan_name already exists!') {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .patch(
            process.env.REACT_APP_SOURCE_URL + '/dataMasterPlans/' + ID,
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              master_plan_name: this.state.ministryName.trim(),
              ...
              master_plan_start: this.state.startYear,
              master_plan_finish: this.state.endYear,
              master_plan_status: this.state.status,
            },
            { headers: { Authorization: `Bearer ${token_id}` } },
          )
          .then(res => {
                ...
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data.map((id, i) => (
                    <tr key={'table' + i}>
                      ...
                      <td>{id.master_plan_name}</td>
                      <td style={{ textAlign: 'center' }}>
                        {id.master_plan_start}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {id.master_plan_finish}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Tooltip title="แก้ไข">
                          <span
                            className="edit-button"
                            onClick={() => this.onEdit(id.master_plan_id)}
                          >
                        ...
                        <Tooltip title="ลบ">
                          <span
                            ...
                            onClick={() =>
                              this.testAxiosDELETE(id.master_plan_id)
                            }
                          >
