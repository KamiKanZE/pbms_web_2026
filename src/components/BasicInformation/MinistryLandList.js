...
export default class MinistryLandList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
  ...
  onEdit = id => {
    ...
    axios
      ...
      .then(res => {
        ...
        const {
          national_strategy_id,
          national_strategy_name,
          national_strategy_detail,
          national_strategy_start,
          national_strategy_finish,
          national_strategy_status,
        } = res.data.result;
        // .map(id => ({'PlanID':
        ...
        this.setState({
          ...
          // PlanName: plan_name,
          modal: true,
          ID: national_strategy_id,
          isForm: 'edit',
          ministryName: national_strategy_name,
          ministryDetail: national_strategy_detail,
          startYear: national_strategy_start,
          endYear: national_strategy_finish,
          status: national_strategy_status,
          validationError: '',
          validationError1: '',
          validationError2: '',
          validationError3: '',
    ...
    if (e.target.name === 'startYear') {
      if (parseInt(e.target.value) > 0) {
        this.setState({
          ...
          endYear: parseInt(e.target.value) + 19,
          validationError2: '',
        });
      } else {
        this.setState({
      ...
      if (parseInt(e.target.value) > 0) {
        this.setState({
          startYear: parseInt(e.target.value) - 19,
        ...
        axios
          .post(
            process.env.REACT_APP_SOURCE_URL + '/dataNationPlans',
            {
              ...
              national_strategy_name: this.state.ministryName.trim(),
              national_strategy_detail: this.state.ministryDetail,
              national_strategy_start: this.state.startYear,
              national_strategy_finish: this.state.endYear,
            },
          ...
          .catch(err => {
            ...
            if (
              err.response.data ===
              'This national_strategy_name already exists!'
            ) {
            ...
            if (err.response.data === 'this data is active') {
              MySwal.fire({
                title: <strong>บันทึกไม่ข้อมูลสำเร็จ</strong>,
                html: <i>ไม่สามารถลบได้เนื่องจากมีการใช้งานข้อมูลอยู่</i>,
                icon: 'error',
                timer: 1000,
                showConfirmButton: false,
              });
            }
            //ไม่สามารถลบได้เนื่องจากมีการใช้งานข้อมูลอยู่
          });
      } else {
        ...
        axios
          .patch(
            process.env.REACT_APP_SOURCE_URL + '/dataNationPlans/' + ID,
            {
              ...
              national_strategy_start: this.state.startYear,
              national_strategy_finish: this.state.endYear,
              national_strategy_status: this.state.status,
            },
            { headers: { Authorization: `Bearer ${token_id}` } },
          )
          .then(res => {
                ...
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data.map((id, i) => (
                    <tr key={'table' + i}>
                      ...
                      <td>{id.national_strategy_name}</td>
                      <td style={{ textAlign: 'center' }}>
                        {id.national_strategy_start}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {id.national_strategy_finish}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Tooltip title="แก้ไข">
                          <span
                            className="edit-button"
                            onClick={() => this.onEdit(id.national_strategy_id)}
                          >
                        ...
                        <Tooltip title="ลบ">
                          <span
                            ...
                            onClick={() =>
                              this.testAxiosDELETE(id.national_strategy_id)
                            }
                          >
