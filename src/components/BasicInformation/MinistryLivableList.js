...
export default class MinistryLivableList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataLivableCities/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataLivableCities/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataLivableCities/' + id)
      .then(res => {
        ...
        this.setState({
          ...
          ID: non_livable_id,
          isForm: 'edit',
          ministryName: non_livable_name,
          ministryDetail: non_livable_detail,
          startYear: non_livable_start,
          endYear: non_livable_finish,
          status: non_livable_status,
          validationError: '',
          validationError1: '',
          validationError2: '',
          validationError3: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .post(
            process.env.REACT_APP_SOURCE_URL + '/dataLivableCities',
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              non_livable_name: this.state.ministryName.trim(),
              non_livable_detail: this.state.ministryDetail,
              non_livable_start: this.state.startYear,
              non_livable_finish: this.state.endYear,
            },
          ...
          .catch(err => {
            ...
            if (err.response.data === 'This non_livable_name already exists!') {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .patch(
            process.env.REACT_APP_SOURCE_URL + '/dataLivableCities/' + ID,
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              non_livable_name: this.state.ministryName.trim(),
              ...
              non_livable_start: this.state.startYear,
              non_livable_finish: this.state.endYear,
              non_livable_status: this.state.status,
            },
            { headers: { Authorization: `Bearer ${token_id}` } },
          )
          .then(res => {
                ...
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data.map((id, i) => (
                    <tr key={'table' + i}>
                      ...
                      <td>{id.non_livable_name}</td>
                      <td style={{ textAlign: 'center' }}>
                        {id.non_livable_start}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {id.non_livable_finish}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Tooltip title="แก้ไข">
                          <span
                            className="edit-button"
                            onClick={() => this.onEdit(id.non_livable_id)}
                          >
                        ...
                        <Tooltip title="ลบ">
                          <span
                            ...
                            onClick={() =>
                              this.testAxiosDELETE(id.non_livable_id)
                            }
                          >
