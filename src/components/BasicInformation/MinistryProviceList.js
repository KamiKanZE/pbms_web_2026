...
export default class MinistryProviceList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataProvincePlans/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataProvincePlans/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataProvincePlans/' + id)
      .then(res => {
        // const ButgetSource1 = res.data;
        // this.setState({ ButgetSource1 });
        const {
          province_strategy_id,
          province_strategy_name,
          province_strategy_detail,
          province_strategy_start,
          province_strategy_finish,
          province_strategy_status,
        } = res.data;
        // .map(id => ({'PlanID':
        ...
        this.setState({
          ...
          // PlanName: plan_name,
          modal: true,
          ID: province_strategy_id,
          isForm: 'edit',
          ministryName: province_strategy_name,
          ministryDetail: province_strategy_detail,
          startYear: province_strategy_start,
          endYear: province_strategy_finish,
          status: province_strategy_status,
          validationError: '',
          validationError1: '',
          validationError2: '',
          validationError3: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .post(
            process.env.REACT_APP_SOURCE_URL + '/dataProvincePlans',
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              province_strategy_name: this.state.ministryName.trim(),
              province_strategy_detail: this.state.ministryDetail,
              province_strategy_start: this.state.startYear,
              province_strategy_finish: this.state.endYear,
            },
          ...
          .catch(err => {
            ...
            if (
              err.response.data ===
              'This province_strategy_name already exists!'
            ) {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .patch(
            process.env.REACT_APP_SOURCE_URL + '/dataProvincePlans/' + ID,
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              province_strategy_name: this.state.ministryName.trim(),
              ...
              province_strategy_start: this.state.startYear,
              province_strategy_finish: this.state.endYear,
              province_strategy_status: this.state.status,
            },
            { headers: { Authorization: `Bearer ${token_id}` } },
          )
          .then(res => {
                ...
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data.map((id, i) => (
                    <tr key={'table' + i}>
                      ...
                      <td>{id.province_strategy_name}</td>
                      <td style={{ textAlign: 'center' }}>
                        {id.province_strategy_start}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {id.province_strategy_finish}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Tooltip title="แก้ไข">
                          <span
                            className="edit-button"
                            onClick={() => this.onEdit(id.province_strategy_id)}
                          >
                        ...
                        <Tooltip title="ลบ">
                          <span
                            ...
                            onClick={() =>
                              this.testAxiosDELETE(id.province_strategy_id)
                            }
                          >
