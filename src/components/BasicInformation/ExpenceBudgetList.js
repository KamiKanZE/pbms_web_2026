...
export default class ExpenceBudgetList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      dataType: '2',
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataExpenditureTypes/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataExpenditureTypes/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataExpenditureTypes/' + id)
      .then(res => {
        ...
        const {
          ...
          expenditure_type_status,
        } = res.data;
        // .map(id => ({'PlanID':
        //   id.main_expenditure_type_id, 'PlanName':id.main_expenditure_type_name
        // }));
        this.setState({
          // PlanID: expenditure_type_id,
          // PlanName: expenditure_type_name,
          modal: true,
          ID: expenditure_type_id,
          isForm: 'edit',
          dataName: expenditure_type_name,
          // dataDetail: expenditure_type_detail,
          // startYear: development_goal_start,
          // endYear: development_goal_finish,
          status: expenditure_type_status,
          validationError: '',
          validationError1: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      axios
        .post(
          process.env.REACT_APP_SOURCE_URL + '/dataExpenditureTypes',
          {
            // expenditure_type_id: this.state.PlanID.trim(),
            // expenditure_type_name: this.state.PlanName.trim(),
            expenditure_type_name: this.state.dataName.trim(),
            // expenditure_type_detail: this.state.dataDetail,
            // expenditure_type_type: this.state.dataType,
        ...
        .catch(err => {
          if (
            err.response.data === 'This expenditure_type_name already exists!'
          ) {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      axios
        .patch(
          process.env.REACT_APP_SOURCE_URL + '/dataExpenditureTypes/' + ID,
          {
            // expenditure_type_id: this.state.PlanID.trim(),
            // expenditure_type_name: this.state.PlanName.trim(),
            expenditure_type_name: this.state.dataName.trim(),
            // expenditure_type_detail: this.state.dataDetail,
            // expenditure_type_type: this.state.dataType,
            expenditure_type_status: this.state.status,
          },
          { headers: { Authorization: `Bearer ${token_id}` } },
        )
        .then(res => {
            ...
            <Table responsive borderless id="table1">
              ...
              <tbody>
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data
                    //.filter(item => item.expenditure_type_type === this.state.dataType)
                    .map((id, i) => (
                      <tr key={'table' + i}>
                        ...
                        <td>{id.expenditure_type_name}</td>
                        <td style={{ textAlign: 'center' }}>
                          <Tooltip title="แก้ไข">
                            <span
                              className="edit-button"
                              onClick={() =>
                                this.onEdit(id.expenditure_type_id)
                              }
                            >
                          ...
                          <Tooltip title="ลบ">
                            <span
                              ...
                              onClick={() =>
                                this.testAxiosDELETE(id.expenditure_type_id)
                              }
                            >
