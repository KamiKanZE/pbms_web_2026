...
export default class MinistryNonList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataMunicipalityPlans/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataMunicipalityPlans/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      ...
      .then(res => {
        ...
        const {
          municipality_strategy_id,
          municipality_strategy_name,
          municipality_strategy_detail,
          municipality_strategy_start,
          municipality_strategy_finish,
          municipality_strategy_status,
        } = res.data;
        // .map(id => ({'PlanID':
        ...
        this.setState({
          ...
          // PlanName: plan_name,
          modal: true,
          ID: municipality_strategy_id,
          isForm: 'edit',
          ministryName: municipality_strategy_name,
          ministryDetail: municipality_strategy_detail,
          startYear: municipality_strategy_start,
          endYear: municipality_strategy_finish,
          status: municipality_strategy_status,
          validationError: '',
          validationError1: '',
          validationError2: '',
          validationError3: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .post(
            process.env.REACT_APP_SOURCE_URL + '/dataMunicipalityPlans',
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              municipality_strategy_name: this.state.ministryName.trim(),
              municipality_strategy_detail: this.state.ministryDetail,
              municipality_strategy_start: this.state.startYear,
              municipality_strategy_finish: this.state.endYear,
            },
          ...
          .catch(err => {
            ...
            if (
              err.response.data ===
              'This municipality_strategy_name already exists!'
            ) {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .patch(
            process.env.REACT_APP_SOURCE_URL + '/dataMunicipalityPlans/' + ID,
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              municipality_strategy_name: this.state.ministryName.trim(),
              ...
              municipality_strategy_start: this.state.startYear,
              municipality_strategy_finish: this.state.endYear,
              municipality_strategy_status: this.state.status,
            },
            { headers: { Authorization: `Bearer ${token_id}` } },
          )
          .then(res => {
                ...
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data.map((id, i) => (
                    <tr key={'table' + i}>
                      ...
                      <td>{id.municipality_strategy_name}</td>
                      <td style={{ textAlign: 'center' }}>
                        {id.municipality_strategy_start}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {id.municipality_strategy_finish}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Tooltip title="แก้ไข">
                          <span
                            ...
                            onClick={() =>
                              this.onEdit(id.municipality_strategy_id)
                            }
                          >
                        ...
                        <Tooltip title="ลบ">
                          <span
                            ...
                            onClick={() =>
                              this.testAxiosDELETE(id.municipality_strategy_id)
                            }
                          >
                        ...
                        <Tooltip title="ดูข้อมูล">
                          <Link
                            to={`/MinistryData/MinistryNon/SubStrategy/${id.municipality_strategy_id}`}
                          >
