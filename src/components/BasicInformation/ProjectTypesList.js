...
export default class ProjectTypesList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
  ...
  onEdit = id => {
    ...
    axios
      ...
      .then(res => {
        ...
        // .map(id => ({'PlanID':
        //   id.main_project_type_id, 'PlanName':id.main_project_type_name
        // }));
        this.setState({
          // PlanID: project_type_id,
          // PlanName: project_type_name,
          modal: true,
          ID: project_type_id,
          isForm: 'edit',
          dataName: project_type_name,
          // dataDetail: project_type_detail,
    ...
    if (
      ...
      axios
        .post(
          ...
          {
            // project_type_id: this.state.PlanID.trim(),
            // project_type_name: this.state.PlanName.trim(),
            project_type_name: this.state.dataName.trim(),
            // project_type_detail: this.state.dataDetail,
            // project_type_type: this.state.dataType,
        ...
        .catch(err => {
          if (
            err.response.data === 'This project_type_name already exists!' ||
      ...
      axios
        .patch(
          process.env.REACT_APP_SOURCE_URL + '/dataProjectTypes/' + ID,
          {
            ...
            // project_type_detail: this.state.dataDetail,
            // project_type_type: this.state.dataType,
            project_type_status: this.state.status,
          },
          { headers: { Authorization: `Bearer ${token_id}` } },
        )
        .then(res => {
        ...
        .catch(err => {
          if (err.response.data === 'This project_type_name already exists!') {
            const validationError1 = 'ชื่อข้อมูลซ้ำ กรุณากรอกใหม่';
            ...
            <Table responsive borderless id="table1">
              ...
              <tbody>
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data
                    //.filter(item => item.project_type_type === this.state.dataType)
                    .map((id, i) => (
                      <tr key={'table' + i}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <Tooltip title="แก้ไข">
                            <span
                              ...
                              onClick={() => this.onEdit(id.project_type_id)}
                            >
                          ...
                          <Tooltip title="ลบ">
                            <span
                              ...
                              onClick={() =>
                                this.testAxiosDELETE(id.project_type_id)
                              }
                            >
