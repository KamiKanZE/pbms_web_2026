...
export default class MinistryITMasterPlanList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataITMasterPlans/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataITMasterPlans/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataITMasterPlans/' + id)
      .then(res => {
        ...
        const {
          it_master_plan_id,
          it_master_plan_name,
          it_master_plan_detail,
          it_master_plan_start,
          it_master_plan_finish,
          it_master_plan_status,
        } = res.data.result;
        // .map(id => ({'PlanID':
        ...
        this.setState({
          ...
          // PlanName: plan_name,
          modal: true,
          ID: it_master_plan_id,
          isForm: 'edit',
          ministryName: it_master_plan_name,
          ministryDetail: it_master_plan_detail,
          startYear: it_master_plan_start,
          endYear: it_master_plan_finish,
          status: it_master_plan_status,
          validationError: '',
          validationError1: '',
          validationError2: '',
          validationError3: '',
  ...
  handleSubmit1 = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .post(
            process.env.REACT_APP_SOURCE_URL + '/dataITMasterPlans',
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              it_master_plan_name: this.state.ministryName.trim(),
              it_master_plan_detail: this.state.ministryDetail,
              it_master_plan_start: this.state.startYear,
              it_master_plan_finish: this.state.endYear,
            },
          ...
          .catch(err => {
            ...
            if (
              err.response.data === 'This it_master_plan_name already exists!'
            ) {
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      if (parseInt(this.state.startYear) < parseInt(this.state.endYear)) {
        axios
          .patch(
            process.env.REACT_APP_SOURCE_URL + '/dataITMasterPlans/' + ID,
            {
              // plan_id: this.state.PlanID.trim(),
              // plan_name: this.state.PlanName.trim(),
              it_master_plan_name: this.state.ministryName.trim(),
              ...
              it_master_plan_start: this.state.startYear,
              it_master_plan_finish: this.state.endYear,
              it_master_plan_status: this.state.status,
            },
            { headers: { Authorization: `Bearer ${token_id}` } },
          )
          .then(res => {
                ...
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data.map((id, i) => (
                    <tr key={'table' + i}>
                      ...
                      <td>{id.it_master_plan_name}</td>
                      <td style={{ textAlign: 'center' }}>
                        {id.it_master_plan_start}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {id.it_master_plan_finish}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Tooltip title="แก้ไข">
                          <span
                            className="edit-button"
                            onClick={() => this.onEdit(id.it_master_plan_id)}
                          >
                        ...
                        <Tooltip title="ลบ">
                          <span
                            ...
                            onClick={() =>
                              this.testAxiosDELETE(id.it_master_plan_id)
                            }
                          >
