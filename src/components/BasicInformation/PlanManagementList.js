...
export default class PlanManagementList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
      ...
      axios
        .post(
          process.env.REACT_APP_SOURCE_URL + '/dataPlans',
          {
            ...
            plan_name: this.state.planName.trim(),
            plan_detail: this.state.planDetail,
        ...
        .catch(err => {
          if (err.response.data === 'This plan_name already exists!') {
      ...
      axios
        .patch(
          process.env.REACT_APP_SOURCE_URL + '/dataPlans/' + ID,
          {
            ...
            plan_name: this.state.planName.trim(),
            plan_detail:
              this.state.planDetail === '' ? '-' : this.state.planDetail,
            plan_type: this.state.planType,
            plan_status: this.state.status,
          },
          { headers: { Authorization: `Bearer ${token_id}` } },
        )
        .then(res => {
            ...
            <Table responsive borderless id="table1">
              ...
              <tbody>
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data
                    //.filter(item => item.plan_type === this.state.planType)
                    .map((id, i) => (
                      <tr key={'table' + i}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <Tooltip title="แก้ไข">
                            <span
                              ...
                              onClick={() => this.onEdit(id.plan_id)}
                            >
                          ...
                          <Tooltip title="ลบ">
                            <span
                              ...
                              onClick={() => this.testAxiosDELETE(id.plan_id)}
                            >
