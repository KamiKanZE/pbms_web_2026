...
export default class PolicyPresidentList extends React.Component {
  constructor(props) {
    super(props);
    let id;
    {
    ...
    this.state = {
      ...
      policyType: '2',
      status: 1,
      url: process.env.REACT_APP_SOURCE_URL + '/dataPolicies/list',
    };
  }
  toggle = modalType => () => {
    if (!modalType) {
  ...
  DELETE = id => {
    axios
      .delete(process.env.REACT_APP_SOURCE_URL + '/dataPolicies/' + id)
      .then(res => {
        // confirmAlert({
  ...
  onEdit = id => {
    ...
    axios
      .get(process.env.REACT_APP_SOURCE_URL + '/dataPolicies/' + id)
      .then(res => {
        ...
        // .map(id => ({'PlanID':
        //   id.main_policy_id, 'PlanName':id.main_policy_name
        // }));
        this.setState({
          // PlanID: policy_id,
          // PlanName: policy_name,
          modal: true,
          ID: policy_id,
          isForm: 'edit',
  ...
  handleSubmit1 = e => {
    e.preventDefault();
    if (
      this.state.policyName.trim()
      // this.state.startYear &&
      // this.state.endYear
    ) {
      axios
        .post(
          process.env.REACT_APP_SOURCE_URL + '/dataPolicies',
          {
            // policy_id: this.state.PlanID.trim(),
            // policy_name: this.state.PlanName.trim(),
            policy_name: this.state.policyName.trim(),
            policy_detail: this.state.policyDetail,
            // policy_type: this.state.policyType,
        ...
        .catch(err => {
          if (err.response.data === 'This policy_name already exists!') {
      ...
      const validationError1 = this.state.policyName.trim()
        ? ''
        : 'กรุณากรอกข้อมูลให้ครบถ้วน';
  ...
  handleUpdate = e => {
    ...
    if (
      ...
      axios
        .patch(
          process.env.REACT_APP_SOURCE_URL + '/dataPolicies/' + ID,
          {
            // policy_id: this.state.PlanID.trim(),
            // policy_name: this.state.PlanName.trim(),
            policy_name: this.state.policyName.trim(),
            policy_detail:
              this.state.policyDetail === '' ? '-' : this.state.policyDetail,
            // policy_type: this.state.policyType,
            policy_status: this.state.status,
          },
          { headers: { Authorization: `Bearer ${token_id}` } },
        )
        .then(res => {
            ...
            <Table responsive borderless id="table1">
              ...
              <tbody>
                {this.state.data.length == 0 ? (
                  ...
                  this.state.data
                    //.filter(item => item.policy_type === this.state.policyType)
                    .map((id, i) => (
                      <tr key={'table' + i}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <Tooltip title="แก้ไข">
                            <span
                              ...
                              onClick={() => this.onEdit(id.policy_id)}
                            >
                          ...
                          <Tooltip title="ลบ">
                            <span
                              ...
                              onClick={() => this.testAxiosDELETE(id.policy_id)}
                            >
